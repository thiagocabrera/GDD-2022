...
PRIMARY KEY(ESCUDERIA_ID, CIRCUITO_ID, TIPO_SECTOR_ID, INCIDENTE_ID, ANIO)
);
...
SECTOR_ID INT NOT NULL REFERENCES LOCRO.BI_SECTOR,
...
PRIMARY KEY (AUTO_ID, CARRERA_ID, VUELTA_ID, SECTOR_ID)
);
...
BEGIN
	...
	GROUP BY AUTO_ESCUDERIA, CARRERA_CIRCUITO, SECTOR_TIPO, i.INCIDENTE_ID, INCIDENTE_TIPO, TIEMPO_ANIO
...
BEGIN
	...
	GROUP BY AUTO_ID, AUTO_ESCUDERIA, CARRERA_ID, CARRERA_CIRCUITO, VUELTA_ID, SECTOR_ID, SECTOR_TIPO, TIEMPO_ID, VUELTA_NRO
...
AS
	...
	WHERE h.CIRCUITO_ID IN (	SELECT TOP 3 CIRCUITO_ID
							...
							ORDER BY COUNT(DISTINCT INCIDENTE_ID) DESC)
...
AS
	...
	COUNT(DISTINCT INCIDENTE_ID) / COUNT(DISTINCT h.ANIO) AS 'PROMEDIO'
	FROM LOCRO.BI_HECHOS_INCIDENTES h
...
IF EXISTS(SELECT [name] FROM sys.views WHERE [name] = 'BI_MAYOR_CONSUMO_PROMEDIO')
DROP VIEW LOCRO.BI_MAYOR_CONSUMO_PROMEDIO
GO
CREATE VIEW LOCRO.BI_MAYOR_CONSUMO_PROMEDIO
AS
	SELECT TOP 3
	c.CIRCUITO_ID AS 'CODIGO CIRCUITO',
	c.CIRCUITO_NOMBRE AS 'CIRCUITO',
	SUM(CONSUMO) / COUNT(DISTINCT STR(AUTO_ID) + STR(CARRERA_ID)) AS 'CONSUMO PROMEDIO'
	FROM LOCRO.BI_HECHOS_TELEMETRIAS h
	JOIN LOCRO.BI_CIRCUITO c
		ON c.CIRCUITO_ID = h.CIRCUITO_ID
	GROUP BY c.CIRCUITO_ID, c.CIRCUITO_NOMBRE
	ORDER BY 2 DESC
GO

IF EXISTS(SELECT [name] FROM sys.views WHERE [name] = 'BI_MEJOR_TIEMPO_DE_VUELTA')
DROP VIEW LOCRO.BI_MEJOR_TIEMPO_DE_VUELTA
GO
CREATE VIEW LOCRO.BI_MEJOR_TIEMPO_DE_VUELTA
AS
	SELECT
	h1.ESCUDERIA_ID AS 'CODIGO ESCUDERIA',
	e.ESCUDERIA_NOMBRE AS 'NOMBRE ESCUDERIA',
	h1.CIRCUITO_ID AS 'CODIGO CIRCUITO',
	c.CIRCUITO_NOMBRE AS 'NOMBRE CIRCUITO',
	TIEMPO_ANIO AS 'ANIO', 
	(SELECT TOP 1 SUM(TIEMPO)
	FROM LOCRO.BI_HECHOS_TELEMETRIAS h2
	JOIN LOCRO.BI_TIEMPO t2 ON t2.TIEMPO_ID = h2.TIEMPO_ID
	WHERE h2.ESCUDERIA_ID = h1.ESCUDERIA_ID AND t2.TIEMPO_ANIO = t1.TIEMPO_ANIO AND h2.CIRCUITO_ID = h1.CIRCUITO_ID
	GROUP BY AUTO_ID, CARRERA_ID, VUELTA_ID
	HAVING SUM(TIEMPO) > 0
	ORDER BY 1 ASC) AS 'MEJOR TIEMPO DE VUELTA'
	FROM LOCRO.BI_HECHOS_TELEMETRIAS h1
	JOIN LOCRO.BI_TIEMPO t1 ON t1.TIEMPO_ID = h1.TIEMPO_ID
	JOIN LOCRO.BI_ESCUDERIA e ON e.ESCUDERIA_ID = h1.ESCUDERIA_ID
	JOIN LOCRO.BI_CIRCUITO c ON c.CIRCUITO_ID = h1.CIRCUITO_ID
	GROUP BY h1.ESCUDERIA_ID, e.ESCUDERIA_NOMBRE, h1.CIRCUITO_ID, c.CIRCUITO_NOMBRE, TIEMPO_ANIO
GO

IF EXISTS(SELECT [name] FROM sys.views WHERE [name] = 'BI_DESGASTE_PROMEDIO')
DROP VIEW LOCRO.BI_DESGASTE_PROMEDIO
GO
CREATE VIEW LOCRO.BI_DESGASTE_PROMEDIO
AS
	SELECT
	AUTO_ID AS 'AUTO',
	VUELTA_NRO AS 'NUMERO DE VUELTA',
	VUELTA_CARRERA AS 'CARRERA',
	h.CIRCUITO_ID AS 'CODIGO CIRCUITO',
	c.CIRCUITO_NOMBRE AS 'NOMBRE CIRCUITO',
	SUM(DESGASTE_NEUMATICOS) / COUNT(*) AS 'DESGASTE NEUMATICOS',
	SUM(DESGASTE_FRENOS) / COUNT(*) AS 'DESGASTE FRENOS',
	SUM(DESGASTE_CAJA) / COUNT(*) AS 'DESGASTE CAJA',
	SUM(DESGASTE_MOTOR) / COUNT(*) AS 'DESGASTE MOTOR'
	FROM LOCRO.BI_HECHOS_TELEMETRIAS h
	JOIN LOCRO.BI_VUELTA v ON v.VUELTA_ID = h.VUELTA_ID
	JOIN LOCRO.BI_CIRCUITO c ON c.CIRCUITO_ID = h.CIRCUITO_ID
	GROUP BY AUTO_ID, VUELTA_NRO, VUELTA_CARRERA, h.CIRCUITO_ID, c.CIRCUITO_NOMBRE
GO
...
SELECT * FROM LOCRO.BI_MAYOR_CONSUMO_PROMEDIO
SELECT * FROM LOCRO.BI_MEJOR_TIEMPO_DE_VUELTA
SELECT * FROM LOCRO.BI_DESGASTE_PROMEDIO
